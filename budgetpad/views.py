...
def dashboard(request):
    ...
    if request.session.has_key('is_logged'):
        ...
        if profile_completed:
            expenses_by_category = AddExpense_info.objects.filter(
                user=request.user).values('category').annotate(total_amount=Sum('amount'))

            categories = []
            amounts = []
            for expense in expenses_by_category:
                categories.append(expense['category'])
                amounts.append(float(expense['total_amount']))

            # Generate the pie chart data
            chart_data_json = generate_pie_chart(request)
            ...
            return render(request,
                          'budgetpad/dashboard.html', context)
        else:
...
def generate_pie_chart(request):
    # Retrieve the user's expense data from the database
    user = request.user
    expenses = AddExpense_info.objects.filter(user=user)
...
def expenses(request):
    ...
    """if request.session.has_key('is_logged'):
        ...
        if sort_by == 'category':
            ...
            expenses = AddExpense_info.objects.filter(
                user=user).order_by(F('date').asc(nulls_last=True))
        paginator = Paginator(expenses, 10)
        page_number = request.GET.get('page')
        page_obj = paginator.get_page(page_number)
        context = {
...
def expenseHistory(request):
    ...
    if request.session.has_key('is_logged'):
        ...
        if profile_completed:
            ...
            # if profile is completed
            return render(request,
                          'budgetpad/expenses-history.html', context)
